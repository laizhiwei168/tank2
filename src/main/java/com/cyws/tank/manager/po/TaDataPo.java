package com.cyws.tank.manager.po;


public class TaDataPo {
	private String EQUIPMENT_ID;
	private String DA_UP;
	private Long id;
	private Double LEVELS;
	private Double LONGITUDE;
	private Double LATITUDE;
	private Integer DIRECTION;
	private Double TEMPERATURE;
	private Double SPEED;
	private Integer HEIGTH;
	private Double MILEAGE;
	private Double PRESSURE;
	private Integer VACUUM;
	private Integer DOOR;
	private Double GAS;
	private Integer DIP_X;
	private Double R_LATITUDE;
	private Double R_LONGITUDE;
	private Double FUEL;
	private Integer IS_LOCATION;
	private Double ACCELERATOR_X;
	private Integer DIP_Y;
	private Double ACCELERATOR_Y;
	private Double CURRENT_VOLUME;
	private Double ELECTRIC;
	private Double TEMPERATURE1;
	private Double TEMPERATURE2;
	private Double TEMPERATURE3;
	private Double TEMPERATURE4;
	private Integer DOOR1;
	private Integer DOOR2;
	private Integer DOOR3;
	private Integer DOOR4;
	private Double VOLTAGE1;
	private Double VOLTAGE2;
	private Double VOLTAGE3;
	private Double VOLTAGE4;
	private Double RSSI1;
	private Double RSSI2;
	private Double RSSI3;
	private Double RSSI4;
	private String PLACE;
	private String STATE;
	private Double PRESSURE1;
	private Double PRESSURE2;
	private Double PRESSURE3;
	private Double PRESSURE4;
	private Double STAND_ADD_UP;
	private Double STAND_ADD_UP1;	
	private Double LEVELS1;
	private Double CURRENT_VOLUME1;
	private Double WORK_ADD_UP;
	private Double WORK_ADD_UP1;
	private Double STAND_FLOW;
	private Double STAND_FLOW1;
	private Double WORK_FLOW;
	private Double WORK_FLOW1;
	private Double GAS1;
	private Double A1;
	private Double A2;
	private Double A3;
	private Double A4;
	private Double A5;
	private Double A6;
	private Double B1;
	private Double B2;
	private Double B3;
	private Double B4;
	private Double B5;
	private Double B6;
	private Double B7;
	private Double C1;
	private Double C2;
	private Double C3;
	private Double C4;
	private Double C5;
	private Double F91;
	private Double F92;
	private Double F93;	
	
	public Double use_volume;
	public Double add_volume;
	
	private Double STOREUNITNUM;
	
	private Double TANKPRESSURE3;
	private Double LEVELS3;	
	private Double TANKPRESSURE4;
	private Double LEVELS4;
	
	// 特殊添加
	private Double LEVEL_MAX;

	// 加臭次数
	private Double addSmellyNumber;


	public Double getAddSmellyNumber() {
		return addSmellyNumber;
	}

	public void setAddSmellyNumber(Double addSmellyNumber) {
		this.addSmellyNumber = addSmellyNumber;
	}

	public Double getTANKPRESSURE3() {
		return TANKPRESSURE3;
	}
	public void setTANKPRESSURE3(Double tANKPRESSURE3) {
		TANKPRESSURE3 = tANKPRESSURE3;
	}
	public Double getLEVELS3() {
		return LEVELS3;
	}
	public void setLEVELS3(Double lEVELS3) {
		LEVELS3 = lEVELS3;
	}
	public Double getTANKPRESSURE4() {
		return TANKPRESSURE4;
	}
	public void setTANKPRESSURE4(Double tANKPRESSURE4) {
		TANKPRESSURE4 = tANKPRESSURE4;
	}
	public Double getLEVELS4() {
		return LEVELS4;
	}
	public void setLEVELS4(Double lEVELS4) {
		LEVELS4 = lEVELS4;
	}
	public Double getSTOREUNITNUM() {
		return STOREUNITNUM;
	}
	public void setSTOREUNITNUM(Double sTOREUNITNUM) {
		STOREUNITNUM = sTOREUNITNUM;
	}	
	public Double getUse_volume() {
		return use_volume;
	}
	public void setUse_volume(Double use_volume) {
		this.use_volume = use_volume;
	}
	public Double getAdd_volume() {
		return add_volume;
	}
	public void setAdd_volume(Double add_volume) {
		this.add_volume = add_volume;
	}
	public Integer getDIRECTION() {
		return DIRECTION;
	}
	public void setDIRECTION(Integer dIRECTION) {
		DIRECTION = dIRECTION;
	}
	public Integer getHEIGTH() {
		return HEIGTH;
	}
	public void setHEIGTH(Integer hEIGTH) {
		HEIGTH = hEIGTH;
	}
	public Integer getVACUUM() {
		return VACUUM;
	}
	public void setVACUUM(Integer vACUUM) {
		VACUUM = vACUUM;
	}
	public Integer getDOOR() {
		return DOOR;
	}
	public void setDOOR(Integer dOOR) {
		DOOR = dOOR;
	}
	public Integer getDIP_X() {
		return DIP_X;
	}
	public void setDIP_X(Integer dIP_X) {
		DIP_X = dIP_X;
	}
	public Integer getIS_LOCATION() {
		return IS_LOCATION;
	}
	public void setIS_LOCATION(Integer iS_LOCATION) {
		IS_LOCATION = iS_LOCATION;
	}
	public Integer getDIP_Y() {
		return DIP_Y;
	}
	public void setDIP_Y(Integer dIP_Y) {
		DIP_Y = dIP_Y;
	}
	public Integer getDOOR1() {
		return DOOR1;
	}
	public void setDOOR1(Integer dOOR1) {
		DOOR1 = dOOR1;
	}
	public Integer getDOOR2() {
		return DOOR2;
	}
	public void setDOOR2(Integer dOOR2) {
		DOOR2 = dOOR2;
	}
	public Integer getDOOR3() {
		return DOOR3;
	}
	public void setDOOR3(Integer dOOR3) {
		DOOR3 = dOOR3;
	}
	public Integer getDOOR4() {
		return DOOR4;
	}
	public void setDOOR4(Integer dOOR4) {
		DOOR4 = dOOR4;
	}
	public Double getPRESSURE1() {
		return PRESSURE1;
	}
	public void setPRESSURE1(Double pRESSURE1) {
		PRESSURE1 = pRESSURE1;
	}
	public Double getPRESSURE2() {
		return PRESSURE2;
	}
	public void setPRESSURE2(Double pRESSURE2) {
		PRESSURE2 = pRESSURE2;
	}
	public Double getPRESSURE3() {
		return PRESSURE3;
	}
	public void setPRESSURE3(Double pRESSURE3) {
		PRESSURE3 = pRESSURE3;
	}
	public Double getPRESSURE4() {
		return PRESSURE4;
	}
	public void setPRESSURE4(Double pRESSURE4) {
		PRESSURE4 = pRESSURE4;
	}
	public Double getLEVELS1() {
		return LEVELS1;
	}
	public void setLEVELS1(Double lEVELS1) {
		LEVELS1 = lEVELS1;
	}
	public Double getA1() {
		return A1;
	}
	public void setA1(Double a1) {
		A1 = a1;
	}
	public Double getA2() {
		return A2;
	}
	public void setA2(Double a2) {
		A2 = a2;
	}
	public Double getA3() {
		return A3;
	}
	public void setA3(Double a3) {
		A3 = a3;
	}
	public Double getA4() {
		return A4;
	}
	public void setA4(Double a4) {
		A4 = a4;
	}
	public Double getA5() {
		return A5;
	}
	public void setA5(Double a5) {
		A5 = a5;
	}
	public Double getA6() {
		return A6;
	}
	public void setA6(Double a6) {
		A6 = a6;
	}
	public String getEQUIPMENT_ID() {
		return EQUIPMENT_ID;
	}
	public void setEQUIPMENT_ID(String eQUIPMENT_ID) {
		EQUIPMENT_ID = eQUIPMENT_ID;
	}
	public String getDA_UP() {
		return DA_UP;
	}
	public void setDA_UP(String dA_UP) {
		DA_UP = dA_UP;
	}
	public Long getId() {
		return id;
	}
	public void setId(Long id) {
		this.id = id;
	}
	public Double getLEVELS() {
		return LEVELS;
	}
	public void setLEVELS(Double lEVELS) {
		LEVELS = lEVELS;
	}
	public Double getLONGITUDE() {
		return LONGITUDE;
	}
	public void setLONGITUDE(Double lONGITUDE) {
		LONGITUDE = lONGITUDE;
	}
	public Double getLATITUDE() {
		return LATITUDE;
	}
	public void setLATITUDE(Double lATITUDE) {
		LATITUDE = lATITUDE;
	}
	
	public Double getTEMPERATURE() {
		return TEMPERATURE;
	}
	public void setTEMPERATURE(Double tEMPERATURE) {
		TEMPERATURE = tEMPERATURE;
	}
	public Double getSPEED() {
		return SPEED;
	}
	public void setSPEED(Double sPEED) {
		SPEED = sPEED;
	}
	
	public Double getMILEAGE() {
		return MILEAGE;
	}
	public void setMILEAGE(Double mILEAGE) {
		MILEAGE = mILEAGE;
	}
	public Double getPRESSURE() {
		return PRESSURE;
	}
	public void setPRESSURE(Double pRESSURE) {
		PRESSURE = pRESSURE;
	}
	
	public Double getGAS() {
		return GAS;
	}
	public void setGAS(Double gAS) {
		GAS = gAS;
	}
	
	public Double getR_LATITUDE() {
		return R_LATITUDE;
	}
	public void setR_LATITUDE(Double r_LATITUDE) {
		R_LATITUDE = r_LATITUDE;
	}
	public Double getR_LONGITUDE() {
		return R_LONGITUDE;
	}
	public void setR_LONGITUDE(Double r_LONGITUDE) {
		R_LONGITUDE = r_LONGITUDE;
	}
	public Double getFUEL() {
		return FUEL;
	}
	public void setFUEL(Double fUEL) {
		FUEL = fUEL;
	}
	
	public Double getACCELERATOR_X() {
		return ACCELERATOR_X;
	}
	public void setACCELERATOR_X(Double aCCELERATOR_X) {
		ACCELERATOR_X = aCCELERATOR_X;
	}
	
	public Double getACCELERATOR_Y() {
		return ACCELERATOR_Y;
	}
	public void setACCELERATOR_Y(Double aCCELERATOR_Y) {
		ACCELERATOR_Y = aCCELERATOR_Y;
	}
	public Double getCURRENT_VOLUME() {
		return CURRENT_VOLUME;
	}
	public void setCURRENT_VOLUME(Double cURRENT_VOLUME) {
		CURRENT_VOLUME = cURRENT_VOLUME;
	}
	public Double getELECTRIC() {
		return ELECTRIC;
	}
	public void setELECTRIC(Double eLECTRIC) {
		ELECTRIC = eLECTRIC;
	}
	public Double getTEMPERATURE1() {
		return TEMPERATURE1;
	}
	public void setTEMPERATURE1(Double tEMPERATURE1) {
		TEMPERATURE1 = tEMPERATURE1;
	}
	public Double getTEMPERATURE2() {
		return TEMPERATURE2;
	}
	public void setTEMPERATURE2(Double tEMPERATURE2) {
		TEMPERATURE2 = tEMPERATURE2;
	}
	public Double getTEMPERATURE3() {
		return TEMPERATURE3;
	}
	public void setTEMPERATURE3(Double tEMPERATURE3) {
		TEMPERATURE3 = tEMPERATURE3;
	}
	public Double getTEMPERATURE4() {
		return TEMPERATURE4;
	}
	public void setTEMPERATURE4(Double tEMPERATURE4) {
		TEMPERATURE4 = tEMPERATURE4;
	}
	
	public Double getVOLTAGE1() {
		return VOLTAGE1;
	}
	public void setVOLTAGE1(Double vOLTAGE1) {
		VOLTAGE1 = vOLTAGE1;
	}
	public Double getVOLTAGE2() {
		return VOLTAGE2;
	}
	public void setVOLTAGE2(Double vOLTAGE2) {
		VOLTAGE2 = vOLTAGE2;
	}
	public Double getVOLTAGE3() {
		return VOLTAGE3;
	}
	public void setVOLTAGE3(Double vOLTAGE3) {
		VOLTAGE3 = vOLTAGE3;
	}
	public Double getVOLTAGE4() {
		return VOLTAGE4;
	}
	public void setVOLTAGE4(Double vOLTAGE4) {
		VOLTAGE4 = vOLTAGE4;
	}
	public Double getRSSI1() {
		return RSSI1;
	}
	public void setRSSI1(Double rSSI1) {
		RSSI1 = rSSI1;
	}
	public Double getRSSI2() {
		return RSSI2;
	}
	public void setRSSI2(Double rSSI2) {
		RSSI2 = rSSI2;
	}
	public Double getRSSI3() {
		return RSSI3;
	}
	public void setRSSI3(Double rSSI3) {
		RSSI3 = rSSI3;
	}
	public Double getRSSI4() {
		return RSSI4;
	}
	public void setRSSI4(Double rSSI4) {
		RSSI4 = rSSI4;
	}
	public String getPLACE() {
		return PLACE;
	}
	public void setPLACE(String pLACE) {
		PLACE = pLACE;
	}
	public String getSTATE() {
		return STATE;
	}
	public void setSTATE(String sTATE) {
		STATE = sTATE;
	}
	
	public Double getSTAND_ADD_UP() {
		return STAND_ADD_UP;
	}
	public void setSTAND_ADD_UP(Double sTAND_ADD_UP) {
		STAND_ADD_UP = sTAND_ADD_UP;
	}
	public Double getSTAND_ADD_UP1() {
		return STAND_ADD_UP1;
	}
	public void setSTAND_ADD_UP1(Double sTAND_ADD_UP1) {
		STAND_ADD_UP1 = sTAND_ADD_UP1;
	}
	
	public Double getCURRENT_VOLUME1() {
		return CURRENT_VOLUME1;
	}
	public void setCURRENT_VOLUME1(Double cURRENT_VOLUME1) {
		CURRENT_VOLUME1 = cURRENT_VOLUME1;
	}
	public Double getWORK_ADD_UP() {
		return WORK_ADD_UP;
	}
	public void setWORK_ADD_UP(Double wORK_ADD_UP) {
		WORK_ADD_UP = wORK_ADD_UP;
	}
	public Double getWORK_ADD_UP1() {
		return WORK_ADD_UP1;
	}
	public void setWORK_ADD_UP1(Double wORK_ADD_UP1) {
		WORK_ADD_UP1 = wORK_ADD_UP1;
	}
	public Double getSTAND_FLOW() {
		return STAND_FLOW;
	}
	public void setSTAND_FLOW(Double sTAND_FLOW) {
		STAND_FLOW = sTAND_FLOW;
	}
	public Double getSTAND_FLOW1() {
		return STAND_FLOW1;
	}
	public void setSTAND_FLOW1(Double sTAND_FLOW1) {
		STAND_FLOW1 = sTAND_FLOW1;
	}
	public Double getWORK_FLOW() {
		return WORK_FLOW;
	}
	public void setWORK_FLOW(Double wORK_FLOW) {
		WORK_FLOW = wORK_FLOW;
	}
	public Double getWORK_FLOW1() {
		return WORK_FLOW1;
	}
	public void setWORK_FLOW1(Double wORK_FLOW1) {
		WORK_FLOW1 = wORK_FLOW1;
	}
	public Double getGAS1() {
		return GAS1;
	}
	public void setGAS1(Double gAS1) {
		GAS1 = gAS1;
	}
	
	public Double getB1() {
		return B1;
	}
	public void setB1(Double b1) {
		B1 = b1;
	}
	public Double getB2() {
		return B2;
	}
	public void setB2(Double b2) {
		B2 = b2;
	}
	public Double getB3() {
		return B3;
	}
	public void setB3(Double b3) {
		B3 = b3;
	}
	public Double getB4() {
		return B4;
	}
	public void setB4(Double b4) {
		B4 = b4;
	}
	public Double getB5() {
		return B5;
	}
	public void setB5(Double b5) {
		B5 = b5;
	}
	public Double getB6() {
		return B6;
	}
	public void setB6(Double b6) {
		B6 = b6;
	}
	public Double getB7() {
		return B7;
	}
	public void setB7(Double b7) {
		B7 = b7;
	}
	public Double getC1() {
		return C1;
	}
	public void setC1(Double c1) {
		C1 = c1;
	}
	public Double getC2() {
		return C2;
	}
	public void setC2(Double c2) {
		C2 = c2;
	}
	public Double getC3() {
		return C3;
	}
	public void setC3(Double c3) {
		C3 = c3;
	}
	public Double getC4() {
		return C4;
	}
	public void setC4(Double c4) {
		C4 = c4;
	}
	public Double getC5() {
		return C5;
	}
	public void setC5(Double c5) {
		C5 = c5;
	}
	public Double getF91() {
		return F91;
	}
	public void setF91(Double f91) {
		F91 = f91;
	}
	public Double getF92() {
		return F92;
	}
	public void setF92(Double f92) {
		F92 = f92;
	}
	public Double getF93() {
		return F93;
	}
	public void setF93(Double f93) {
		F93 = f93;
	}
	public Double getLEVEL_MAX() {
		return LEVEL_MAX;
	}
	public void setLEVEL_MAX(Double lEVEL_MAX) {
		LEVEL_MAX = lEVEL_MAX;
	}
	
}
