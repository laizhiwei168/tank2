...
public class BitOperator {
	...
	public float byte2Float(byte[] bs,int len) {
		int value=0;
		for(int i=0;i<len;i++){			
			value= value+(bs[i] & 0xFF)<< i*8;			
		}
		return Float.intBitsToFloat(value);
	}
	
	public Double byte2Double(byte[] bs) {
		long result=0;
		int len = bs.length;
		int temp;
		for (int i = 0; i < len; i++) {
			...
			if (temp == 0) {
				...
				result += (bs[i] & 0x0ff) << temp;
			}
		}
		return Double.valueOf(result);//Float.intBitsToFloat(value);
	}
	...
	public int RX_CheckSum(byte[] bs, int start, int end)
	{ 
